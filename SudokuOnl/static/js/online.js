...
socket.on("connect", () => {
  ...
  // Reset trạng thái game khi kết nối lại
  resetGameState();
});
...
function handleInput(event) {
  ...
  if (!isCorrect) {
    ...
    if (isOnlineMode && value !== 0) {
      // Tăng điểm khi điền đúng
      score += 10;
      updateMoveHistory(row, col, value);
      updateScoreDisplay(); // Cập nhật hiển thị điểm ngay lập tức
    }
    ...
    if (checkBoardCompletion()) {
      ...
      showModal("Chúc mừng!", "Bạn đã hoàn thành bảng Sudoku!");
      stopTimer();
    }
  }
...
function updateMoveHistory(row, col, value) {
  ...
  const historyElement = document.querySelector(".move-history");
  if (historyElement) {
    // Tạo container cho lịch sử nếu chưa có
    if (!historyElement.querySelector(".history-container")) {
    ...
    const container = historyElement.querySelector(".history-container");
    const time = formatTime(Math.floor((move.timestamp - startTime) / 1000));

    // Tạo phần tử hiển thị bước đi
    const moveDiv = document.createElement("div");
    moveDiv.className = "move-item";
    moveDiv.style.padding = "5px";
    moveDiv.style.marginBottom = "5px";
    moveDiv.style.borderBottom = "1px solid #dee2e6";

    // Định dạng thông tin bước đi
    const moveText = document.createElement("span");
    moveText.innerHTML = `<strong>Bước ${
      moveHistory.length
    }:</strong> Điền số ${value} vào ô [${row + 1},${
      col + 1
    }] <small>(${time})</small>`;
    moveText.style.color = "#495057";

    moveDiv.appendChild(moveText);
    container.appendChild(moveDiv);

    // Tự động cuộn xuống bước đi mới nhất
