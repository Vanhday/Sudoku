...
def handle_join_game():
    ...
    logger.info(f"Player {player_id} trying to join game")
    ...
    if available_room_id:
        logger.info(f"Found available room {available_room_id}")
        # Người chơi thứ 2
        rooms_data[available_room_id]['players'].append(player_id)
        rooms_data[available_room_id]['scores'][player_id] = 0
        rooms_data[available_room_id]['boards'][player_id] = [row[:] for row in rooms_data[available_room_id]['boards'][rooms_data[available_room_id]['players'][0]]]
        rooms_data[available_room_id]['history'][player_id] = []
        join_room(available_room_id, sid=player_id)

        # Gửi thông tin game cho cả hai người chơi
        for pid in rooms_data[available_room_id]['players']:
            opponent_id = next(p for p in rooms_data[available_room_id]['players'] if p != pid)
            logger.info(f"Sending match found to player {pid}")
            emit('match_found', {
                'room_id': available_room_id,
                'board': rooms_data[available_room_id]['boards'][pid],
                'solution': rooms_data[available_room_id]['solution'],
                'opponent_board': rooms_data[available_room_id]['boards'][opponent_id],
                'level': rooms_data[available_room_id]['level']
            }, room=pid)
    else:
        logger.info(f"No available room found for player {player_id}")
        # Không có phòng sẵn, tạo phòng mới
        room_id = str(uuid.uuid4())
        game_board, solution_board = generate_sudoku('medium')
        rooms_data[room_id] = {
            'players': [player_id],
            'boards': {player_id: [row[:] for row in game_board]},
        ...
        emit('waiting', {'message': 'Đang chờ người chơi thứ hai...'}, room=room_id)
...
def handle_request_new_game(data):
    ...
    # Tìm phòng có sẵn với cùng cấp độ
    available_room_id = None
    for room_id, room_data in rooms_data.items():
        if len(room_data['players']) < 2 and room_data['level'] == level:
            available_room_id = room_id
            logger.info(f"Found available room {room_id} with level {level}")
            break

    if available_room_id:
        logger.info(f"Joining player {player_id} to room {available_room_id}")
        # Người chơi thứ 2
        rooms_data[available_room_id]['players'].append(player_id)
        rooms_data[available_room_id]['scores'][player_id] = 0
        rooms_data[available_room_id]['boards'][player_id] = [row[:] for row in rooms_data[available_room_id]['boards'][rooms_data[available_room_id]['players'][0]]]
        ...
        for pid in rooms_data[available_room_id]['players']:
            opponent_id = next(p for p in rooms_data[available_room_id]['players'] if p != pid)
            logger.info(f"Sending match found to player {pid} with opponent {opponent_id}")
            emit('match_found', {
                'room_id': available_room_id,
                'board': rooms_data[available_room_id]['boards'][pid],
                'solution': rooms_data[available_room_id]['solution'],
        ...
        logger.info(f"Creating new room for player {player_id} with level {level}")
        # Không có phòng sẵn, tạo phòng mới
        room_id = str(uuid.uuid4())
        game_board, solution_board = generate_sudoku(level)
        rooms_data[room_id] = {
